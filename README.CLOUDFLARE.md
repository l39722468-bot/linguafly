...
> `npm run cf:deploy` and `npm run cf:upload` run. A plain `next build`
> (or `wrangler build`) is **not** enough: it only produces `.next/` and
> deploy fails with `The entry-point file at ".open-next/worker.js" was not found.`

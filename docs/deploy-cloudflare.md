...
- `NEXT_PUBLIC_GOOGLE_TAG_GATEWAY_PATH=/gtag` (ruta first-party reservada; el HTML sigue usando googletagmanager.com)
- `NEXT_PUBLIC_GOOGLE_TAG_GATEWAY_INLINE=1` solo si el rewrite de zona está apagado y el snippet debe usar `src="/gtag/"`
- `INDEXNOW_*` si aplica

### Google Tag Gateway (Cloudflare)

El snippet carga la URL oficial (`https://www.googletagmanager.com/gtag/js?id=G-ZNL3VGHK2E`) + `gtag('config', 'G-ZNL3VGHK2E')`. Cloudflare Tag Gateway reescribe esa petición a first-party `/gtag/`. **No pongas `src="/gtag/"` en el HTML**: el panel inyecta el loader y PageSpeed lo descarga dos veces (~180 KiB × 2).

En el panel **Google Tag Gateway for linguafly.app**:

1. Toggle **on**.
2. Measurement ID: `G-ZNL3VGHK2E`.
3. Measurement path: **`/gtag`** — no uses `/metrics` (el Worker legado responde JSON ahí).
4. **Desactiva «Set up tag» / configurar etiqueta automática.** Ya emitimos Consent Mode + gtag en el layout; la inyección extra duplica el JS. Google pide apagar el setup automático si usas Consent Mode.
5. **Actualizar configuración** si la ruta del panel no es `/gtag`.

/**
 * Unidad 30 B1 — Repaso 26–29
 * Contenido mixto: Quantifiers, Both/either/neither, Articles, Reflexive pronouns
 * Vocabulario: Food & drink, Choices, Places: buildings, Personal experiences
 */

import { UNIT_30_ALL_EXERCISES } from './unit-30-lessons-index';

export const UNIT_TITLE = 'Repaso 26–29';

/** Todos los ejercicios de la Unidad 30 (5 lecciones). */
export const UNIT_30_EXERCISES = UNIT_30_ALL_EXERCISES;

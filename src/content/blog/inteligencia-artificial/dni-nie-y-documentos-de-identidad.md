...
title: "DNI, NIE y documentos de identidad"
description: >-
  DNI en un chatbot: ni foto ni número. Un modelo no es un registro ni un KYC. Qué hacer si ya lo pegaste y por qué no extrae campos de un DNI.
readTime: 11 min
keywords:
  - dni en un chatbot
  - foto del dni y ia
...
excerpt: >-
  Ni foto ni número. Un chatbot no es un registro ni un KYC. Si ya lo
...
faqs:
  ...
  - question: ¿Es más seguro enviar una foto del DNI que escribir el número?
    answer: "No. La foto lleva el número, el nombre, la caducidad, la foto de cara y a menudo la firma y la dirección. Es más dato, no menos. El clip y el Ctrl+V son el mismo tipo de envío."
  - question: ¿Un chatbot puede hacer de KYC o de registro?
    answer: "No. Un modelo de lenguaje no es un registro civil, ni la Policía, ni el cumplimiento de un banco. Si un chat te pide el DNI ‘para verificarte’, cierras. Si tú se lo das ‘para ver si pasaría un KYC’, también cierras: no hay verificación, hay un documento en un servidor ajeno."
  - question: ¿Qué hago si ya he pegado el DNI o subido la foto?
    answer: "Paras. No lo vuelves a enviar ‘para que lo borre’ o ‘ahora en modo temporal’. Borras el hilo en la interfaz (gesto, no garantía). Si la web no era la oficial, cambias la contraseña de esa cuenta y la del correo si la escribiste. No sigas pegando el documento porque ‘ya está fuera’."
  - question: ¿El chat temporal lava un DNI?
    answer: "No. El texto o la foto salen igual de tu máquina para que te respondan. El modo que no guarda el hilo en la barra no convierte el documento en inocuo. El DNI no entra. Ni ahí."
---

**DNI en un chatbot** no es un atajo de formularios. Es un envío: número, nombre, caducidad, foto de cara y, si subes imagen, firma y soporte físico. Un modelo de lenguaje no es un registro, no es la Policía y no es un KYC. No extrae “solo los campos” en tu aparato. Lee lo que le mandas en las máquinas de otra empresa. Esta página es el satélite de identidad. La lista general de qué no pegar está en [privacidad al usar IA: qué no pegar nunca](/blog/inteligencia-artificial/privacidad-al-usar-ia-que-no-pegar-nunca). Aquí el trabajo es uno: el documento oficial no entra. Ni foto. Ni número. Ni “un momento”.
...
1. **“Solo quiero que copie los campos al formulario.”** Copias tú. El plástico está en tu mesa. El modelo no te ahorra un minuto que merezca el riesgo de un documento oficial.
2. **“Le pido que no lo guarde.”** El prompt no es un contrato. La política del producto manda. Y aun con entrenamiento apagado, el mensaje viaja para contestarte.
3. **“Es mi DNI, no el de otro.”** Sigue siendo un identificador. Sigue saliendo de tu máquina. El hecho de que el titular seas tú no convierte el chat en tu carpeta del cajón.
...
1. Abres el formulario en una ventana.
2. El DNI, en la mesa. Tú lees. Tú escribes.
3. Si el PDF del *formulario* (no el del DNI) es un lío de casillas, puedes pedir al chat *sin documento*: “Explícame qué suele ir en la casilla ‘número de soporte’ de un alta. No me pidas el mío. No inventes el de nadie.”
4. Si no sabes qué casilla es, preguntas al anfitrión o al organismo, no al modelo con la foto.
...
1. El portal, abierto. El DNI, en la mesa.
2. Si hay un PDF *de la oferta* (público, sin tus datos), ese PDF puede resumirse recortado, con las reglas de [subir un PDF a un chatbot: qué hacer y qué no](/blog/inteligencia-artificial/subir-un-pdf-a-un-chatbot-que-hacer-y-que-no). El DNI no se adjunta “de propina”.
3. CV sin número de documento: plantilla con huecos. No inventes un DNI de ejemplo que parezca real.
...
1. **Paras.** No mandes un segundo mensaje con “borra lo anterior” y el DNI otra vez “para que sepas qué borrar”. No abras [chat temporal](/blog/inteligencia-artificial/chat-temporal-o-modo-que-no-guarda) para repetir el pegado “sin historial”. El texto sigue saliendo.
2. **No sigas el hilo.** No pidas JSON, no pidas “ahora anónimalo”, no pidas “valida si es verdadero”. Cierra la conversación.
3. **Borras el chat en la interfaz** si el producto lo permite. Es un gesto útil. No es un certificado de borrado en todos los servidores. Trátalo como tal.
4. **Contraseñas, si hace falta.** El DNI no se “cambia”. Sí rotas claves si *también* pegaste login, códigos SMS o el correo, o si la web no era la oficial.
5. **Web falsa.** Asume que alguien tiene la imagen. Cambias la contraseña de esa cuenta y la del correo si la reutilizaste. Si hay indicios de uso del documento, el cauce es entidad o denuncia, no otro prompt. Aquí no hay dictamen de qué trámite te obliga.
6. **No “ya da igual”.** Cada pegado es otro envío. No reenvíes la foto a un segundo modelo “para que me diga el riesgo”. Ya ha viajado.

---
title: 'Must vs Have To: Diferencias, Uso y Ejemplos'
date: '2026-02-07'
updatedDate: '2026-09-04'
author: linguafly-team
description: >-
  La diferencia entre must y have to no es solo de forma — cambia quién impone la obligación. Y en negativo, la diferencia es todavía más importante: mustn't...
category: gramatica
alt: Estudiante aprendiendo la diferencia entre must y have to en inglés
readTime: 12 min
keywords:
  - 'Must vs Have To: Diferencias, Uso y Ejemplos'
  - must have to diferencia ingles
  - diferencia entre must y have to con ejemplos
  - mustnt y dont have to ejercicios gratis
  - cómo expresar obligación y prohibición en inglés
  - ejercicios de must have to para B1 B2
  - cuándo usar must o have to en inglés
  - must y have to ejercicios con respuestas
  - diferencia entre mustnt y dont have to
  - obligación interna y externa en inglés
excerpt: >-
  La diferencia entre must y have to no es solo de forma — cambia quién impone la obligación. Y en negativo, la diferencia es todavía más importante: mustn't y don't have to significan cosas completamente distintas.
faqs:
  - question: ¿Puedo siempre usar have to en lugar de must?
    answer: >-
      En la mayoría de contextos sí, especialmente en inglés hablado. Pero hay casos donde must es la única opción: instrucciones escritas formales, obligaciones muy personales y deducciones lógicas ("You must be tired after that journey"). En esos contextos, have to suena raro o cambia el matiz.
  - question: ¿Cómo se dice "must" en otros tiempos verbales?
    answer: >-
      Must no tiene forma de pasado ni de futuro propios. Para otros tiempos usas "have to": "I had to work late yesterday" (pasado), "I'll have to call them tomorrow" (futuro), "I've had to redo the whole report" (present perfect). Must solo funciona en presente.
  - question: ¿Cuál es la diferencia entre mustn't y don't have to?
    answer: >-
      Es una de las confusiones más frecuentes. "Mustn't" indica prohibición — algo que no puedes o no debes hacer. "Don't have to" indica ausencia de obligación — algo que no es necesario, pero puedes hacerlo si quieres. "You mustn't eat here" (está prohibido). "You don't have to eat here" (no es obligatorio, pero puedes).
  - question: ¿Have got to es lo mismo que have to?
    answer: >-
      Sí, en la práctica tienen el mismo significado. "Have got to" es más informal y más frecuente en el inglés británico. "I've got to go" = "I have to go". En inglés americano se usa más "have to" en el habla cotidiana.
  - question: ¿Cuál es la diferencia entre must have done y had to?
    answer: >-
      Son estructuras completamente distintas. "Must have done" es una deducción sobre el pasado ("She must have left early — her car is gone"). "Had to" es obligación en pasado ("I had to work on Saturday"). No se pueden intercambiar.
  - question: ¿Se usa must igual en inglés británico y americano?
    answer: >-
      Hay diferencias de frecuencia. En inglés americano, "have to" domina claramente en el habla cotidiana y "must" suena más formal o literario. En inglés británico, "must" sigue siendo habitual en registros neutros. Para exámenes Cambridge o IELTS, ambas formas son correctas.
canonical: 'https://linguafly.app/blog/gramatica/must-have-to-diferencia'
---

Poca gente nota la diferencia entre *must* y *have to* la primera vez que los estudia. Los dos se traducen como "tener que" o "deber" al español, y en muchos casos sí son intercambiables. Pero en otros, la diferencia importa — y en negativo, la diferencia es enorme.

Antes de entrar en los detalles, si quieres ver cómo estos dos modales encajan dentro del sistema completo de verbos modales, la guía [Verbos Modales en Inglés](/blog/gramatica/verbos-modales-ingles-guia) te da el panorama general.

---

## La diferencia clave: ¿quién impone la obligación?

La distinción más importante entre **must** y **have to** tiene que ver con el origen de la obligación.

**Must** → obligación interna. Tú decides que algo es necesario. Viene de ti.

**Have to** → obligación externa. Una norma, una persona, una circunstancia te lo impone.

Ejemplos donde la diferencia se nota:

- *I must call my mother.* — Siento que debo hacerlo, es una obligación moral o personal.
- *I have to call my mother.* — Ella me lo ha pedido, o hay alguna razón externa.

- *You must try this restaurant.* — Te lo recomiendo con entusiasmo, es mi opinión.
- *You have to wear a mask in this area.* — Es la norma del lugar.

En muchos contextos cotidianos, esta distinción no importa mucho y los hablantes nativos los mezclan. Pero en situaciones formales, instrucciones escritas o cuando quieres ser preciso, la diferencia sí cuenta.

---

## Cuándo usar must

### 1. Obligación personal o interna

Cuando tú mismo decides que algo es necesario o importante.

- *I really must start exercising more regularly.*
- *We must remember to lock the door.*
- *I must finish this before I leave.*

El tono es más determinado o urgente que *have to* en estos casos.

### 2. Instrucciones y normas escritas

En carteles, manuales, documentos oficiales, *must* es la forma más frecuente.

- *Passengers must wear seatbelts at all times.*
- *All visitors must sign in at reception.*
- *Applications must be submitted before 31 March.*

Aquí no hay una persona concreta imponiendo la norma — es el contexto institucional el que la establece. Este uso aparece constantemente en exámenes Cambridge (B2 First, C1 Advanced) porque evalúa precisamente si el alumno distingue registro.

### 3. Deducciones lógicas (certeza)

Cuando algo es casi seguro a partir de la evidencia que tienes.

- *You must be exhausted — you've been working all day.*
- *She must know the answer — she's the expert.*
- *There must be a mistake in the invoice.*

Este uso no tiene nada que ver con obligación. Es pura deducción lógica: la conclusión más razonable a partir de lo que observas. *Have to* no funciona aquí — no puedes decir *"You have to be exhausted"* con sentido de deducción.

### 4. Recomendaciones entusiastas

*Must* también aparece en recomendaciones donde quieres transmitir urgencia o entusiasmo:

- *You must see this film — it's incredible.*
- *You must try the local food when you visit.*
- *This is a book you must read before the course.*

El tono es diferente al de *should* (más suave) o *have to* (más externo). *Must* aquí suena como una convicción personal fuerte.

---

## Cuándo usar have to

### 1. Obligación externa

Cuando la norma viene de fuera: leyes, reglas de empresa, horarios, situaciones.

- *I have to wear a uniform at work.*
- *You have to be 18 to vote in most countries.*
- *She has to commute two hours every day.*

### 2. Todos los tiempos verbales donde must no funciona

Must solo tiene forma de presente. Para pasado, futuro y otros tiempos, usas *have to*:

- *I had to stay late at the office yesterday.* (pasado)
- *She'll have to redo the presentation.* (futuro)
- *They've had to cancel the trip.* (present perfect)
- *He might have to see a specialist.* (con otro modal)

Este es uno de los errores más frecuentes en el nivel B1-B2: los estudiantes intentan conjugar *must* en pasado y escriben *"I musted"* o *"I mused"*, que no son formas válidas en inglés.

### 3. Preguntas sobre obligación

Aunque es posible preguntar con *must* (*Must I attend this meeting?*), suena muy formal y algo anticuado. En la práctica, se usa *have to*:

- *Do I have to attend this meeting?*
- *Do we have to pay in advance?*
- *Does she have to work on Saturdays?*

### 4. Obligaciones de terceros

Cuando hablas de la obligación de otra persona que viene de una causa externa, *have to* es más natural:

- *My doctor says I have to cut down on salt.*
- *The contract says we have to deliver by Friday.*
- *He has to attend mandatory training every year.*

---

## Tabla comparativa completa: must vs have to

| Situación | Must | Have to |
|-----------|------|---------|
| Obligación interna (tú decides) | ✓ preferido | ✓ posible |
| Obligación externa (norma, ley) | ✓ posible | ✓ preferido |
| Instrucciones escritas formales | ✓ preferido | menos frecuente |
| Preguntas | formal/anticuado | ✓ natural |
| Pasado | ✗ no existe | ✓ had to |
| Futuro | ✗ no existe | ✓ will have to |
| Present perfect | ✗ no existe | ✓ have had to |
| Con otro modal | ✗ no existe | ✓ might/could have to |
| Deducción lógica | ✓ exclusivo | ✗ no funciona |
| Recomendación entusiasta | ✓ habitual | ✓ posible |

---

## En negativo: la diferencia es crítica

Este es el punto donde más errores se cometen, y donde la distinción entre los dos es más importante.

| Forma | Significado | Ejemplo |
|-------|-------------|---------|
| **Mustn't** | Prohibición — no está permitido | *You mustn't park here* |
| **Don't have to** | No es necesario, pero puedes | *You don't have to come if you're busy* |

Los significados son casi opuestos. Confundirlos da lugar a malentendidos importantes.

- *You mustn't tell anyone.* → Está prohibido decírselo a alguien.
- *You don't have to tell anyone.* → No es obligatorio, pero puedes si quieres.

- *Children mustn't run near the pool.* → Regla de seguridad — está prohibido.
- *You don't have to run — we have plenty of time.* → No hace falta correr.

Más ejemplos de *mustn't* (prohibición):
- *You mustn't touch the exhibits.* (museo)
- *Patients mustn't eat for 8 hours before surgery.*
- *Staff must not take photos in this area.*

Más ejemplos de *don't have to* (ausencia de obligación):
- *You don't have to dress up — it's a casual event.*
- *She doesn't have to pay rent — she owns the apartment.*
- *We don't have to decide right now.*

### Por qué se confunden tanto en español

El problema para los hispanohablantes es que en español, "no tener que" y "no deber" a veces se usan de forma intercambiable coloquialmente. Pero en inglés la distinción es absoluta: *mustn't* es prohibición activa, *don't have to* es simplemente ausencia de obligación.

---

## Needn't: otra forma de expresar que algo no es necesario

En inglés británico, *needn't* funciona de forma similar a *don't have to*:

- *You needn't worry about it.* = *You don't have to worry about it.*
- *She needn't come if she's unwell.* = *She doesn't have to come if she's unwell.*

*Needn't* suena más formal y es más frecuente en escritura o en el inglés británico. En conversación, *don't have to* es más natural en la mayoría de contextos.

---

## Must y have to en contextos de probabilidad (solo must)

Un uso que solo tiene *must*, no *have to*, es el de deducción o probabilidad alta:

- *She must be very intelligent to pass that exam.*
- *It must be cold outside — everyone's wearing a coat.*
- *He must have forgotten the meeting.*

Para la deducción negativa se usa *can't*, no *mustn't*:

- ❌ *He mustn't be at home — the lights are off.*
- ✓ *He can't be at home — the lights are off.*

*Mustn't* es prohibición, no deducción negativa.

---

## Must have done vs had to: la distinción en pasado

Aquí está otra confusión muy frecuente, especialmente en B2 y C1.

**Must have done** → deducción sobre el pasado (no sabes con certeza lo que pasó, pero lo deduces).

- *She must have left early — her desk is empty.*
- *They must have taken the wrong turn.*
- *He must have misunderstood the instructions.*

**Had to** → obligación real en el pasado (algo que efectivamente ocurrió).

- *I had to work all weekend to finish the project.*
- *She had to take a taxi because she missed the bus.*
- *We had to cancel the meeting at the last minute.*

No son intercambiables. *"She had to left early"* es incorrecto gramaticalmente. *"I must have worked all weekend"* implica que no estás seguro de si trabajaste, lo cual cambia completamente el sentido.

Este par es uno de los puntos más evaluados en el Use of English del Cambridge B2 First y C1 Advanced.

---

## Diferencias entre inglés británico y americano

El inglés americano usa *have to* con mucha más frecuencia en situaciones cotidianas donde el inglés británico usaría *must*. Algunas diferencias prácticas:

**Inglés americano** (más informal, más *have to*):
- *You have to try this!* (recomendación)
- *I have to say, that was impressive.*
- *You have to be kidding me.*

**Inglés británico** (más *must* en registros neutros):
- *You must try this!*
- *I must say, that was impressive.*
- *You must be joking.*

Ninguna es incorrecta, pero si preparas un examen británico (Cambridge, IELTS), el uso de *must* en registros formales y recomendaciones entusiastas es más esperable.

---

## Errores más frecuentes de hispanohablantes

### Error 1: Usar must en pasado

- ❌ *Yesterday I must go to the doctor.*
- ✓ *Yesterday I had to go to the doctor.*

### Error 2: Confundir mustn't con don't have to

- ❌ *You mustn't come if you don't want to.* (suena como una prohibición, no como una opción)
- ✓ *You don't have to come if you don't want to.*

### Error 3: Usar have to para deducción

- ❌ *It has to be cold outside — everyone's wearing a coat.*
- ✓ *It must be cold outside — everyone's wearing a coat.*

### Error 4: Usar must have done como obligación pasada

- ❌ *Last year I must have studied a lot for the exam.*
- ✓ *Last year I had to study a lot for the exam.*

---

## Must en la voz pasiva

Cuando combinas *must* con la voz pasiva, la estructura es: **must + be + participio**.

- *All forms must be completed in full.*
- *Visitors must be accompanied at all times.*
- *The report must be submitted by Friday.*

Si quieres profundizar en este uso, el artículo sobre [pasiva con verbos modales](/blog/gramatica/pasiva-modales-ingles) cubre *must*, *should*, *can* y *might* en estructuras pasivas con ejemplos para examen.

---

## Ejercicios de práctica

Elige entre *must*, *have to*, *mustn't* o *don't have to*:

1. *You _____ wear a seatbelt — it's the law.* → **have to**
2. *You _____ tell anyone about this — it's confidential.* → **mustn't**
3. *You _____ bring anything — we have everything ready.* → **don't have to**
4. *She looks pale. She _____ be feeling well.* → **mustn't** ❌ → **can't** ✓ (deducción negativa)
5. *I _____ call the bank yesterday — there was a problem with my account.* → **had to**
6. *He _____ have taken the train — his car was in the garage.* → **must have**
7. *All employees _____ attend the annual review.* → **must / have to**

---

## Resumen práctico

Cuando dudes entre los dos, hazte estas preguntas:

1. **¿La obligación viene de mí o de fuera?** → Must (interna) / Have to (externa)
2. **¿Estás en pasado o futuro?** → Solo have to (had to / will have to)
3. **¿Es una pregunta sobre si es obligatorio?** → Do... have to?
4. **¿Es negativo?** → Mustn't (prohibición) / Don't have to (no es necesario)
5. **¿Es una deducción lógica?** → Solo must (positivo) / can't (negativo)
6. **¿Es deducción sobre el pasado?** → Must have done (no had to)

Con estas seis preguntas tienes cubierta la gran mayoría de situaciones donde *must* y *have to* generan dudas.

Si quieres seguir con otros modales que expresan posibilidad e incertidumbre, el artículo sobre [may y might en inglés](/blog/gramatica/may-might-ingles) es el siguiente paso lógico.

## Practica en el curso Linguafly

Si estás siguiendo el curso por unidades, estas guías conectan este tema con la práctica guiada:

- [A1 U41 — Aeropuerto: must / should](/blog/curso-a1/unidad-41-aeropuerto-must-should)

## Fuentes

- Cambridge Dictionary: https://dictionary.cambridge.org/
- British Council LearnEnglish: https://www.britishcouncil.org/learnenglish
- CEFR/MCER (Council of Europe): https://www.coe.int/en/web/common-european-framework-reference-languages

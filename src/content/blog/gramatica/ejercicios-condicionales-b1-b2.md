---
title: 'Ejercicios de Condicionales B1-B2: Second y Third Conditional'
description: >-
  Ejercicios de Second y Third Conditional para niveles B1-B2 con soluciones.
  Practica transformaciones y correcciones en el artículo y descarga el contenido en PDF.
readTime: 10 min
excerpt: >-
  Hoja práctica de Second y Third Conditional para B1-B2 con ejercicios resueltos.
  Practica en el artículo y descárgalo en PDF.
keywords:
  - ejercicios condicionales b1 b2
  - second conditional ejercicios
  - third conditional ejercicios
  - ejercicios de second y third conditional B1 B2
  - condicionales B1 B2 en PDF con respuestas
  - práctica gratis de if clauses de pasado
  - transformar frases con second conditional
  - ejercicios de third conditional para exámenes
  - Ejercicios de Condicionales B1-B2
  - ejercicios condicionales b1 b2 en inglés
author: linguafly-team
date: '2026-07-30'
updatedDate: '2026-08-31'
category: gramatica
canonical: 'https://linguafly.app/blog/gramatica/ejercicios-condicionales-b1-b2'
downloadPdf: true
pdfFileName: 'ejercicios-condicionales-b1-b2'
pdfDownloadLabel: 'Descargar ejercicios en PDF'
faqs:
  - question: '¿Qué es el Second Conditional?'
    answer: 'Expresa hipótesis presentes o irreales. Estructura: If + past simple, would + infinitivo.'
  - question: '¿Qué es el Third Conditional?'
    answer: 'Expresa hipótesis pasadas imposibles. Estructura: If + past perfect, would have + participio.'
---

Práctica de **Second Conditional** y **Third Conditional** con transformaciones y correcciones para niveles B1 y B2.

Para repasar la teoría completa, visita la [guía de condicionales en inglés](/blog/temas/ejercicios-condicionales-ingles).

---

## Bloque 1 — Second Conditional (hipótesis presentes)

Estructura: **If + past simple, would + infinitivo**

### Ejercicio 1: Completa con la forma correcta

1. If I _______ (be) you, I would apologize.
2. She would travel more if she _______ (have) time.
3. If we _______ (win) the lottery, we would buy a house.
4. What would you do if you _______ (lose) your phone?

### Ejercicio 2: Corrige y completa

5. If I would have more money, I would study abroad. → _______________
6. If he were more confident, _______.

### Ejercicio 3: Transforma usando Second Conditional

7. She is shy, so she does not speak in class.
8. I do not know her number, so I cannot call her.

**Soluciones:**
1. were
2. had
3. won
4. lost
5. If I had more money, I would study abroad.
6. he would apply for that job
7. If she were not shy, she would speak in class.
8. If I knew her number, I could call her.

---

## Bloque 2 — Third Conditional (hipótesis pasadas)

Estructura: **If + past perfect, would have + past participle**

### Ejercicio 4: Completa con la forma correcta

1. If they _______ (leave) earlier, they would have caught the train.
2. She would have passed if she _______ (study) more.
3. If I had seen you, I _______ (say) hello.
4. If we had known, we _______ (not come).

### Ejercicio 5: Transforma usando Third Conditional

5. He missed the alarm. He arrived late.
6. I did not save money, so I did not buy the laptop.

### Ejercicio 6: Corrige y escribe

7. If you would have told me, I would have helped. → _______________
8. Escribe una oración en Third Conditional sobre un arrepentimiento del pasado.

**Soluciones:**
1. had left
2. had studied
3. would have said
4. wouldn't have come
5. If he had not missed the alarm, he would have arrived on time.
6. If I had saved money, I would have bought the laptop.
7. If you had told me, I would have helped.
8. Respuesta abierta (ej.: *If I had studied harder, I would have passed the exam.*)

---

## Resumen rápido

| Tipo | Estructura | Uso |
|------|-----------|-----|
| Second | if + past simple → would + inf. | Hipótesis presente irreal |
| Third | if + past perfect → would have + pp. | Hipótesis pasada irreal |

## Más práctica por nivel

- [Ejercicios A2-B1: Zero y First Conditional](/blog/gramatica/ejercicios-condicionales-a2-b1)
- [Ejercicios C1-C2: Mixed Conditionals e inversión](/blog/gramatica/ejercicios-condicionales-c1-c2)
- [Guía completa con los cuatro condicionales](/blog/gramatica/ejercicios-condicionales-ingles-b1-b2)

## Fuentes

- Cambridge Dictionary: https://dictionary.cambridge.org/
- British Council LearnEnglish: https://www.britishcouncil.org/learnenglish

### Producción controlada

Transforma *I don't know her number, so I can't call her* en una hipótesis del
presente: *If I knew her number, I could call her*. Después transforma *He
didn't back up the file, so he lost the data*: *If he had backed up the file,
he wouldn't have lost the data*. Explica por qué la primera usa past simple y
la segunda past perfect. Finalmente escribe una frase con *might* para mostrar
una consecuencia posible, no segura. Revisa que el auxiliar corresponda al
tiempo y que nunca aparezca *would* tras *if*.

Como práctica oral, justifica una decisión hipotética usando *If I were you* y
cuenta un arrepentimiento con *If I had...*. Añade una razón con *because*,
*although* o *so* para conectar las ideas.

## Second, Third y mixed conditional: práctica

El **Second Conditional** imagina una situación improbable: *If I had more
time, I would learn Japanese*. El **Third Conditional** mira hacia un pasado
irreversible: *If we had left earlier, we would have caught the train*. Nunca
pongas *would* después de *if*. Completa: 1) *If I ______ (be) you, I ______
(ask) for help.* 2) *If they ______ (know) about traffic, they ______ (leave)
earlier.* 3) *If I had accepted the job, I ______ (live) abroad now.* 4)
Corrige: *If I would have studied, I passed.*

**Soluciones:** 1) *were, would ask*. 2) *had known, would have left*. 3)
*would be living* (mixed conditional). 4) *If I had studied, I would have
passed the exam*. Para revisar, subraya condición y consecuencia, identifica
el momento temporal y comprueba auxiliar, participio y coma. Acepta *could* o
*might* cuando expresen capacidad o posibilidad, no certeza.

### Reto final

Redacta un mensaje sobre una decisión profesional: explica qué harías si
tuvieras más tiempo, qué habría ocurrido si hubieras elegido otra opción y qué
consecuencia presente mantiene esa decisión. Usa un second, un third y un
mixed conditional, y añade *could* o *might* en una de las consecuencias.
Después revisa cada oración con tres preguntas: ¿la condición pertenece al
presente o al pasado?, ¿el auxiliar coincide con ese tiempo?, ¿aparece
*would* únicamente en la consecuencia? Leer el texto en voz alta ayuda a
detectar participios irregulares y evita mezclar *had* con *would have*.

...
export const metadata: Metadata = {
  title: `Inglés B1 para Camareros | ${SITE_BRAND_NAME}`,
  description: "Curso de inglés B1 para camareros: maneja conversaciones fluidas con clientes internacionales, quejas, cobros y vocabulario avanzado de hostelería.",
  alternates: {
    canonical: getAbsoluteUrl('/curso-camarero-b1'),
  },
};

async function CourseContent() {
  const units = CAMARERO_B1_COURSE.units.map((u: any) =>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #FF6B6B, #FF8E53, #FFA06B)' }}>
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-20">
          <div className="inline-flex items-center gap-2 bg-white/20 border border-white/30 text-white px-4 py-2 rounded-full mb-6">
            ...
            <span className="text-xs font-black tracking-widest uppercase">🍽️ Camarero / Restaurante · Nivel B1 · {level_label}</span>
          </div>
          <h1 className="text-5xl sm:text-6xl font-black text-white tracking-tighter mb-4">
            Inglés B1<br />
            <span className="text-yellow-100">para Camarero</span>
          </h1>
          <p className="text-lg text-white/85 max-w-xl mb-8">
            {units.length} unidades con vocabulario profesional, gramática, escucha y práctica oral para el sector de {"camarero / restaurante"}.
          </p>
          <Link
            href="/{route}/unit-1"
      ...
      <main className="max-w-4xl mx-auto px-6 pb-20">
        ...
        <div className="mt-10 p-6 bg-blue-50 border border-blue-100 rounded-2xl">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {['a1','a2','b1','b2'].map(l => (
              <Link key={l} href={`/curso-camarero-${l}`} className={`px-3 py-1.5 rounded-xl text-sm font-bold border transition-colors ${l === 'b1' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-700 border-slate-200 hover:border-blue-300'}`}>
                {l.toUpperCase()}
              </Link>
            ))}
          </div>

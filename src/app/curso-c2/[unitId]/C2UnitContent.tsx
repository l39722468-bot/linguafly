...
function C2UnitContentInner() {
  ...
  const { totalUnits, coursePath } = useCourseMetadata('ingles-c2');
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="max-w-md w-full p-8 bg-white rounded-2xl shadow-xl text-center">
        ...
        <Link href="/curso-c2" className="inline-block bg-slate-800 text-white px-8 py-3 rounded-xl font-bold hover:bg-slate-900 transition-all">
          Volver al Curso C2
        </Link>
      </div>
    </div>
  ...
  if (exercises.length === 0) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <Link href="/curso-c2" className="text-violet-600 underline">Volver al curso C2</Link>
      </div>
    </div>
  );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="max-w-xl w-full bg-white rounded-[2rem] shadow-2xl p-12 text-center animate-in zoom-in duration-700">
          ...
          <AIExercisePractice
            courseLevel="C2"
            mainTopic={unitTitle || 'Proficiency English'}
            onExercisesReady={handleAIPracticeReady}
          />
          {totalUnits ? (
            <UnitJourneyContinue
              coursePath={coursePath ?? '/curso-c2'}
              currentUnitId={unitId}
              totalUnits={totalUnits}
            />
          ) : (
            <a href="/curso-c2" className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-xl hover:bg-slate-800 transition-all shadow-xl flex items-center justify-center gap-3 transform hover:scale-[1.02] active:scale-95">
    ...
    <div className="min-h-screen bg-slate-50">
      <nav className="bg-white border-b p-4 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-4">
          <Link href="/curso-c2" className="p-2 hover:bg-slate-100 rounded-full transition-colors">
      ...
      <main className="max-w-4xl mx-auto p-4 py-8">
        ...
        <ExerciseRenderer
          ...
          onComplete={(result?: { success: boolean; score: number }) => {
            ...
            if (ex) {
              const topic = ex.topicName || ex.topic || unitTitle || 'Proficiency English';
              recordResult(ex.id ?? `c2-${unitId}-${currentIndex}`, topic, result?.success ?? true, result?.score ?? 100);
            }
...
export default function C2UnitContent() {
  return (

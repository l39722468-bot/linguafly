...
export const metadata: Metadata = {
  title: `Inglés A1 para Logística | ${SITE_BRAND_NAME}`,
  description: "Curso de inglés A1 para logística: vocabulario básico de almacén, mercancías, cargas y comunicación inicial con compañeros de trabajo.",
  alternates: {
    canonical: getAbsoluteUrl('/curso-logistica-a1'),
  },
};

async function CourseContent() {
  const units = LOGISTICA_A1_COURSE.units.map((u: any) =>
      ...
      <header className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #059669, #0891B2, #0EA5E9)' }}>
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-20">
          <div className="inline-flex items-center gap-2 bg-white/20 border border-white/30 text-white px-4 py-2 rounded-full mb-6">
            ...
            <span className="text-xs font-black tracking-widest uppercase">📦 Logística / Almacén · Nivel A1 · {level_label}</span>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 pb-20">
        ...
        <div className="mt-10 p-6 bg-blue-50 border border-blue-100 rounded-2xl">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {['a1','a2','b1','b2'].map(l => (
              <Link key={l} href={`/curso-logistica-${l}`} className={`px-3 py-1.5 rounded-xl text-sm font-bold border transition-colors ${l === 'a1' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-700 border-slate-200 hover:border-blue-300'}`}>
                {l.toUpperCase()}
              </Link>
            ))}
          </div>

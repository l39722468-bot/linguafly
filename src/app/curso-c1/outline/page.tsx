...
export const metadata: Metadata = {
  title: `Temario curso inglés C1 | ${SITE_BRAND_NAME}`,
  description: "Consulta el temario completo del curso de inglés nivel C1 de Linguafly. Revisa las unidades, contenidos de gramática y competencias que aprenderás.",
  alternates: canonicalAlternates('/curso-c1/outline'),
};
...
async function OutlineContent() {
  ...
  try {
    ...
    console.error('[curso-c1/outline] Error loading course metadata:', err);
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
        <div className="max-w-md text-center">
          ...
          <Link href="/curso-c1/outline" className="text-indigo-600 font-bold hover:underline">
            Recargar
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 print-py-4">
          <div className="flex items-center justify-between mb-6 print-mb-3">
            <div className="flex flex-wrap items-center gap-3">
              <Link
                href="/curso-c1"
                className="inline-flex items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-700 transition-colors print-hidden"
              >
                ← Volver al curso C1
              </Link>
            </div>
          ...
          <div className="flex items-start gap-4 mb-4 print-mb-2">
            <div className="w-12 h-12 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center print-hidden">
            ...
            <div>
              ...
              <p className="text-lg text-slate-600 font-medium print-text-sm">
                Vista completa de las {courseMetadata.totalUnits} unidades ·{' '}
                {Math.round(courseMetadata.totalDuration / 60)} horas en total
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 print-hidden">
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              <p className="text-2xl font-black text-slate-900">{courseMetadata.totalUnits}</p>
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              ...
              <p className="text-xs font-bold text-slate-500 uppercase">Ejercicios</p>
            </div>
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              <p className="text-2xl font-black text-slate-900">
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              ...
              <p className="text-xs font-bold text-slate-500 uppercase">Temas</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 print-py-4">
        <CourseOutlineTable units={courseMetadata.units} coursePath="/curso-c1" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <p className="text-slate-600 font-medium">Cargando índice del curso…</p>
      </div>
    </div>
  );
}
